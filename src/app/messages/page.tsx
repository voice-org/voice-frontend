      ...
      {isCalling && selectedChat && (
        <div className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-between p-8 md:p-12 animate-in fade-in zoom-in-95 duration-500 overflow-hidden">
          ...
          <div className="absolute inset-0 z-0">
            <img alt="BG" className="w-full h-full object-cover opacity-20 blur-3xl scale-110" src={selectedChat.user.avatar} />
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center mt-12 sm:mt-20">
            <div className="relative mb-8">
              ...
              <div className="w-40 h-40 sm:w-56 sm:h-56 rounded-full overflow-hidden border-4 border-white/10 shadow-2xl relative">
                <img alt="Caller" className="w-full h-full object-cover" src={selectedChat.user.avatar} />
                {callType === 'video' && (
                   <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <video className="w-full h-full object-cover grayscale opacity-50" autoPlay muted loop />
                   </div>
                )}
              </div>
            </div>
            <h3 className="text-3xl sm:text-5xl font-black text-white mb-2 tracking-tight drop-shadow-lg">{selectedChat.user.name}</h3>
            <p className="text-primary text-lg sm:text-xl font-bold flex items-center gap-2 drop-shadow-md">
              ...
              {callType === 'video' ? 'VOICE Video Calling...' : 'VOICE Calling...'}
            </p>
          </div>
          ...
          <div className="relative z-10 w-full max-w-md flex items-center justify-between gap-4 mb-8 sm:mb-16">
            ...
            <button onClick={endCall} className="p-8 bg-destructive text-white rounded-full hover:bg-destructive/90 transition-all shadow-[0_0_50px_rgba(239,68,68,0.3)] active:scale-90 flex flex-col items-center gap-2">
          ...
          {callType === 'video' && (
            <div className="absolute top-4 right-4 sm:top-10 sm:right-10 w-32 h-44 sm:w-40 sm:h-56 bg-black rounded-2xl border-2 border-white/20 overflow-hidden shadow-2xl z-20">
               <div className="absolute inset-0 bg-primary/20 flex items-center justify-center">
                  <span className="text-[10px] font-bold text-white/50 uppercase">Your Camera</span>
               </div>
            </div>
          )}
        </div>
          ...
          <div className={cn(
            ...
            {selectedChat ? (
              <>
                ...
                <header className="sticky top-0 bg-background/80 backdrop-blur-md border-b border-border dark:border-[#2F3336] p-4 flex items-center justify-between z-10">
                  ...
                  <div className="flex items-center gap-3 text-muted-foreground">
                    <button onClick={() => startCall('audio')} className="p-2 hover:bg-secondary hover:text-primary rounded-full transition-all">
                      <Phone className="w-5 h-5" />
                    </button>
                    <button onClick={() => startCall('video')} className="p-2 hover:bg-secondary hover:text-primary rounded-full transition-all">
                ...
                <footer className="p-3 border-t border-border dark:border-[#2F3336] flex items-center gap-2 bg-background relative">
                  {isRecording ? (
                    <div className="flex-1 flex items-center gap-4 bg-secondary dark:bg-[#16181C] rounded-full px-4 py-2 animate-in slide-in-from-bottom-2">
                      ...
                      <span className="text-sm font-bold text-destructive">Recording {formatTime(recordingTime)}</span>
                      <div className="flex-1 flex items-center gap-1 justify-center">
                         {[1,2,3,4,5,4,3,2,1,2,3].map((h, i) => (
                           <div key={i} className="w-1 bg-primary rounded-full animate-bounce" style={{ height: `${h * 3}px`, animationDelay: `${i * 0.1}s` }}></div>
                         ))}
                      </div>
                      <button onClick={() => setIsRecording(false)} className="text-muted-foreground hover:text-foreground">
                        <X className="w-4 h-4" />
                      </button>
                      <button onClick={handleSendVoice} className="bg-primary text-white p-2 rounded-full shadow-lg">
                    ...
                    <>
                      <div className="flex items-center text-primary">
                        ...
                        <button onClick={() => setIsRecording(true)} className="p-2 hover:bg-primary/10 transition-colors rounded-full text-primary">
                      ...
                      <div className="flex-1 relative">
                        <input 
                          ...
                          onKeyDown={(e) => e.key === 'Enter' && messageText.trim() && (setMessageText(''), toast({ title: "Sent", description: "Message delivered." }))}
                        />
                      </div>
                      <button 
                        disabled={!messageText.trim()}
                        onClick={() => (setMessageText(''), toast({ title: "Sent", description: "Message delivered." }))}
                        className={cn(
                          "p-2.5 rounded-full transition-colors",
                          messageText.trim() ? "bg-primary text-white shadow-lg" : "text-primary opacity-50 cursor-not-allowed"
                        )}
                      >
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-background">
                ...
                <h2 className="text-4xl font-black mb-2 tracking-tighter">Select a message</h2>
                <p className="text-muted-foreground max-w-sm mb-8 text-lg">
                  Choose from your existing conversations, or start a new one to connect.
                </p>
  ...
  return (
    ...
    >
      <div className="relative flex-shrink-0">
        <img alt="Avatar" className="w-12 h-12 rounded-full object-cover border border-border" src={conversation.user.avatar} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1 min-w-0">
            ...
            <span className="text-muted-foreground text-xs truncate ml-1">{conversation.user.handle}</span>
            <span className="text-muted-foreground text-xs flex-shrink-0">· {conversation.timestamp}</span>
          </div>
