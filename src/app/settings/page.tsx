...
type SettingSection = 'account' | 'security' | 'privacy' | 'notifications' | 'display';
    ...
    <div className="bg-background text-foreground min-h-screen">
        ...
        <div className="sm:hidden sticky top-0 bg-background/80 backdrop-blur-md z-50 border-b border-border dark:border-white/10 px-4 py-3 flex items-center justify-between">
          ...
          <Sheet>
            <SheetTrigger asChild>
              <button className="p-1"><Menu className="w-6 h-6 text-muted-foreground" /></button>
            </SheetTrigger>
            <SheetContent side="left" className="w-72 p-0 bg-background">
              ...
              <div className="p-4 space-y-4">
                ...
                <SidebarLink icon={SettingsIcon} label="Settings" active href="/settings" />
              </div>
            </SheetContent>
          </Sheet>
        </div>
      ...
      <div className="container mx-auto max-w-7xl h-screen flex">
        <header className="w-20 xl:w-64 h-full flex flex-col justify-between p-2 xl:px-4 xl:py-4 sticky top-0 border-r border-border dark:border-white/10 overflow-y-auto hidden sm:flex">
          ...
          <div className="flex items-center p-3 rounded-full hover:bg-secondary dark:hover:bg-white/5 transition-colors cursor-pointer xl:w-full mt-4" onClick={handleLogout}>
            <div className="flex items-center gap-3">
              ...
              <div className="hidden xl:flex flex-col leading-tight overflow-hidden">
                ...
                <span className="text-muted-foreground text-sm truncate">Logout</span>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 flex overflow-hidden">
          <div className={cn(
            ...
            <div className="py-2">
              <SectionItem id="account" icon={UserCircle} label="Your Account" description="Account info, password, deactivate" active={activeSection === 'account'} onClick={() => handleSectionChange('account')} />
              <SectionItem id="security" icon={ShieldCheck} label="Security & Account Access" description="Security, apps and sessions" active={activeSection === 'security'} onClick={() => handleSectionChange('security')} />
              <SectionItem id="privacy" icon={Eye} label="Privacy & Safety" description="Manage what you see and share" active={activeSection === 'privacy'} onClick={() => handleSectionChange('privacy')} />
              <SectionItem id="notifications" icon={BellRing} label="Notifications" description="Choose which notifications you receive" active={activeSection === 'notifications'} onClick={() => handleSectionChange('notifications')} />
              <SectionItem id="display" icon={Palette} label="Accessibility, Display & Languages" description="Manage display, color and more" active={activeSection === 'display'} onClick={() => handleSectionChange('display')} />
              <button onClick={handleLogout} className="w-full text-left px-4 py-4 flex items-center gap-4 text-destructive hover:bg-destructive/5 transition-colors">
          ...
          <div className={cn(
            ...
            <div className="sticky top-0 bg-background/80 backdrop-blur-md z-20 px-4 py-4 border-b border-border dark:border-white/10 flex items-center gap-4">
              ...
              }} className={cn("p-1 hover:bg-secondary rounded-full transition-colors", !isMobileDetail && navStack.length === 0 && "hidden")}>
            ...
            <div className="flex-1 relative overflow-x-hidden">
              <div key={activeSubSection || activeSection} className="animate-in fade-in slide-in-from-right-4 duration-300 h-full">
                {!activeSubSection ? (
                  <div className="space-y-1">
                    {activeSection === 'account' && (
                      <>
                        ...
                        <SettingsAction label="Account Information" description="Review your username, email, and phone number" onClick={() => pushNav('Account Information')} />
                        <SettingsAction label="Change your password" description="Change your password at any time" onClick={() => pushNav('Change Password')} />
                        <SettingsAction label="Download an archive of your data" description="Get a copy of your VOICE data" onClick={() => pushNav('Download Data')} />
                        <SettingsAction label="Deactivate your account" description="Find out how you can deactivate your account" variant="destructive" onClick={() => pushNav('Deactivate')} />
                      </>
                    )}
                    ...
                    {activeSection === 'security' && (
                      <>
                        ...
                        <SettingsAction label="Two-factor authentication" description="Help protect your account from being accessed by others" onClick={() => pushNav('Two-factor authentication')} />
                        <SettingsAction label="Connected apps" description="Manage third-party apps connected to your account" onClick={() => pushNav('Connected Apps')} />
                        <SettingsAction label="Sessions" description="Manage your active sessions on other devices" onClick={() => pushNav('Active Sessions')} />
                      </>
                    )}
                    ...
                    {activeSection === 'privacy' && (
                      <>
                        <div className="px-4 py-6 text-sm text-muted-foreground">
                          Manage what information you allow other people on VOICE to see.
                        </div>
                        <div className="px-4 py-4 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer">
                          <div className="flex flex-col pr-4">
                            <span className="font-bold text-sm">Protect your VOICEs</span>
                            <span className="text-xs text-muted-foreground">Only show your posts to people who follow you</span>
                          </div>
                          <Switch />
                        </div>
                        <SettingsAction label="Audience and tagging" description="Manage who can tag you in photos" onClick={() => pushNav('Audience')} />
                        <SettingsAction label="Muted and blocked" description="Manage the accounts you have muted or blocked" onClick={() => pushNav('Safety')} />
                        <SettingsAction label="Direct Messages" description="Control who can send you direct messages" onClick={() => pushNav('DM Privacy')} />
                      </>
                    )}
                    ...
                    {activeSection === 'notifications' && (
                      <>
                        ...
                        <SettingsAction label="Filters" description="Choose which notifications you want to see" onClick={() => pushNav('Filters')} />
                        <SettingsAction label="Preferences" description="Select your preferences by notification type" onClick={() => pushNav('Preferences')} />
                        <div className="px-4 py-4 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer">
                          <div className="flex flex-col pr-4">
                            <span className="font-bold text-sm">Push Notifications</span>
                            <span className="text-xs text-muted-foreground">Receive real-time updates</span>
                          </div>
                          ...
                          <div className="p-6 rounded-2xl border border-border dark:border-white/10 bg-secondary/20 space-y-6">
                            <div className="flex items-center justify-between">
                              <div className="flex flex-col pr-4">
                                ...
                                <span className="text-xs text-muted-foreground">Enjoy a darker interface for low light</span>
                              </div>
                              <Switch 
                                checked={theme === 'dark'} 
...
function SectionItem({ id, icon: Icon, label, description, active, onClick }: { id: string, icon: any, label: string, description: string, active: boolean, onClick: () => void }) {
  return (
...
function SettingsAction({ label, description, variant = "default", onClick }: { label: string, description: string, variant?: "default" | "destructive", onClick?: () => void }) {
  return (
    <div onClick={onClick} className="px-4 py-4 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer transition-colors group">
      <div className="flex flex-col leading-tight">
        <span className={cn("font-bold text-sm", variant === "destructive" ? "text-destructive" : "group-hover:text-primary")}>{label}</span>
...
function ColorOption({ color, hex, active, onClick }: { color: AccentColor, hex: string, active: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-110",
        active ? "ring-2 ring-primary ring-offset-4 ring-offset-background" : "hover:ring-2 hover:ring-primary/50"
      )}
