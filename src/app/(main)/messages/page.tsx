...
export default function MessagesPage() {
  ...
  return (
    <div className="h-full flex overflow-hidden relative">
      ...
      <div
        className={cn(
          "flex-1 w-full border-r border-border dark:border-white/10 h-full overflow-y-auto no-scrollbar transition-all duration-300 ease-in-out",
        ...
        <div className="sticky top-0 bg-background/80 backdrop-blur-md z-40 border-b border-border dark:border-white/10">
          ...
          <div className="px-4 pb-3">
            <div className="relative">
              ...
              <input
                className="w-full bg-secondary dark:bg-white/5 border-none rounded-full py-2.5 pl-12 pr-4 text-sm outline-none focus:ring-1 focus:ring-primary"
      ...
      >
        {selectedChat ? (
          <>
            <header className="sticky top-0 bg-background/80 backdrop-blur-md border-b border-border dark:border-white/10 p-4 flex items-center justify-between z-10">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar relative">
              ...
              <div className="flex justify-start">
                <div className="bg-secondary dark:bg-white/5 p-3 rounded-2xl rounded-tl-none max-w-[80%] text-sm leading-relaxed">
            ...
            <footer className="p-3 border-t border-border dark:border-white/10 flex items-center gap-2 bg-background relative">
              {isRecording ? (
                <div className="flex-1 flex items-center gap-4 bg-secondary dark:bg-white/5 rounded-full px-4 py-2 animate-in slide-in-from-bottom-2">
                  <div className="w-2 h-2 rounded-full bg-destructive animate-pulse"></div>
                  <span className="text-sm font-bold text-destructive">
                    Recording {formatTime(recordingTime)}
                  </span>
                  <div className="flex-1 flex items-center gap-1 justify-center">
                    {[1, 2, 3, 4, 3, 2, 3, 4, 2].map((h, i) => (
                      <div
                        key={i}
                ...
                <>
                  ...
                  <div className="flex-1 relative">
                    <input
                      className="w-full bg-secondary dark:bg-white/5 border-none rounded-full py-2.5 px-4 text-sm outline-none focus:ring-1 focus:ring-primary"
...
function ConversationItem({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "flex gap-3 px-4 py-4 border-b border-border dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer transition-colors relative",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1 min-w-0 font-bold text-sm truncate">
            {conversation.user.name}
            {conversation.user.verified && (
              <VerifiedBadge isMe={conversation.user.handle === "@johndoe"} />
            )}
            <span className="text-muted-foreground text-xs font-normal truncate ml-1">
              {conversation.user.handle} · {conversation.timestamp}
            </span>
          </div>
